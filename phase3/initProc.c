...
int main(){
    ...
    for (i = 1; i <= UPROCMAX; i++)
    {
        ...
        Uproc_pcbs[i - 1]->p_supportStruct->sup_asid = i;

        
        for (int j = 0; j < 32; j++){
            /*set asid in each page table entry to i - the asid is at bit 6 - bit 11*/
            Uproc_pcbs[i-1]->p_supportStruct->uproc_pageTable[j].entryHI = i << 6; 

            /*VPN field (starts from bit 12 to bit 31) will be set to [0x80000000 ... 0x8001E000] to the first 31 entries*/
            Uproc_pcbs[i-1]->p_supportStruct->uproc_pageTable[j].entryHI = (VPNSTART + i) << 12;

            /*each page table entry's D field will be set to 1*/
            Uproc_pcbs[i - 1]->p_supportStruct->uproc_pageTable[j].entryLO |= 1 << 10;

            /*each page table entry's G field will be set to 0 - off*/
            Uproc_pcbs[i - 1]->p_supportStruct->uproc_pageTable[j].entryLO |= 0 << 8;

            /* each page table entry's V bit field will be set to 0 - not valid*/
            Uproc_pcbs[i - 1]->p_supportStruct->uproc_pageTable[j].entryLO |= 0 << 9;
        }
